...
/**
 * - tabClient = tableau répertoriant tous les clients connectés
 ...
 * - mutexTabClient = mutex pour la modification de tabClient[]
...
void afficheSalon(char *pseudoEnvoyeur)
{
	char *chaineAffiche = malloc(sizeof(char) * (TAILLE_DESCRIPTION + TAILLE_NOM_SALON + 50) * 4);
	int place;
	int compteur = 0;
 ...
 *
 * @param nombre le nombre dont on souhaite connaitre la longueur
...
int useOfCommand(char *msg, char *pseudoEnvoyeur)
{
	...
	else if (strcmp(strToken, "/créer") == 0)
	{
		...
		if (numSalon == -1)
		{
			envoiPrive(pseudoEnvoyeur, "Le maximum de salons est atteint, vous ne pouvez pas en créer pour le moment\n");
			return 1;
		}
	...
	else if (strcmp(strToken, "/kick") == 0 || strcmp(strToken, "/quick") == 0)
	{
		...
		if (i == -1)
		{
			envoiPrive(pseudoEnvoyeur, "Le pseudo n'existe pas ou n'est pas connecté\n");
			return 1;
		}
